...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-3xl" 
         x-data="{ 
            type: '{{ old('type', $paymentMethod->type) }}',
            details: {{ json_encode(old('details', $paymentMethod->details ?? [])) }}
         }">
            ...
            <div class="p-6 space-y-8">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $paymentMethod->name) }}" required class="mt-1 shadow-sm focus:ring-orange-500 focus:border-orange-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" x-model="type" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm rounded-md bg-gray-100 cursor-not-allowed">
                            @foreach($types as $key => $label)
                                <option value="{{ $key }}" {{ $key !== $paymentMethod->type ? 'disabled' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-500">Method type cannot be changed after creation.</p>
                ...
                <div>
                    ...
                    <div class="prose max-w-none">
                        <input id="instructions" type="hidden" name="instructions" value="{{ old('instructions', $paymentMethod->instructions) }}">
                ...
                <div class="flex items-start pt-4">
                    <div class="flex items-center h-5">
                        <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $paymentMethod->is_active) ? 'checked' : '' }} class="focus:ring-orange-500 h-4 w-4 text-orange-600 border-gray-300 rounded">
                    </div>
